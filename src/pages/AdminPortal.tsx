...
// ─── Mock Data ─────────────────────────────────────────────
import { stats, sosCases, pendingDocuments, checkinRecords, auditLogs, students, geoData } from "./AdminPortalData";
...
export default function AdminPortal() {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      // Nous simulons la jointure entre profiles et user_roles pour l'instant
      // Dans Supabase, il faudrait créer une Vue (View) sécurisée ou utiliser un appel RPC
      ...
      const formattedAdmins = data.map((item: any) => ({
        id: item.user_id,
        name: item.profiles?.full_name || "Sans nom",
        email: item.profiles?.email || "Email inconnu",
        role: item.role,
        active: true,
      }));

      setDbAdmins(formattedAdmins);
    } catch (error) {
      console.error(error);
      setDbAdmins([]);
    } finally {
  ...
  const filteredSOS = sosFilter === "all" ? sosCases : sosCases.filter(s => s.status === "active");
  ...
  const handleRegisterAdmin = async () => {
    ...
    if (!newAdminEmail || !newAdminName) {
      setRegisterError("L'email et le nom sont requis.");
      return;
    }
    ...
    try {
      // Simulation MVP : on informe l'ambassadeur de la marche à suivre
      setTimeout(() => {
        setRegisterSuccess(
          `Demande d'ajout enregistrée pour ${newAdminName}. Pour finaliser la création dans cette version Serverless, l'administrateur doit s'inscrire sur la page d'accueil avec l'email ${newAdminEmail}, puis vous devrez changer son rôle en "admin" dans Supabase.`
        );
        setNewAdminEmail("");
        setNewAdminPassword("");
        setNewAdminName("");
